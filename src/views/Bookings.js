...
import { UPDATE_BOOKING_RECORD, UPDATE_BOOKING_RECORD_SUCCESS, UPDATE_BOOKING_RECORD_FAILURE, FETCH_BOOKINGS, FETCH_BOOKINGS_SUCCESS, FETCH_BOOKINGS_FAILED } from "../actions/types";
...
export default function Bookings() {
  ...
  const [bookingState, setData] = useState([]);
  ...
  useEffect(() => {
    ...
    if (data && data.getBookings && data.getBookings.bookings.length) {
      dispatch({
        type: FETCH_BOOKINGS_SUCCESS,
        payload: data.getBookings.bookings
      });
    } else {
      dispatch({
  ...
  return (
    ...
    <>
      {loading ? <CircularLoading />
        : bookingState && bookingState.length > 0 &&
        <MaterialTable
          title="Bookings"
          columns={columns}
          data={data.getBookings.bookings}
          editable={{
            // *********************** TEST PART
            onRowAdd: newData =>
              new Promise(resolve => {
